...
def iniciar_animacion(x_tiempoPosicion, x_esperado, x_tiempo, v_esperado):
    ...
    #-------------------------------------- Variables Iniciales ------------------------------------
    anchoP, altoP = 854, 480
    escala = 100  # Factor de escala para ajustar los valores de posición y velocidad a la pantalla de Pygame
    desplazamiento = altoP / 6  # Desplazamiento para mantener la masa dentro de la pantalla en caso de valores negativos
    freq_vibracion = 6  # Frecuencia de vibración del punto de anclaje en Hz
    amplitud_vibracion = 10  # Amplitud de vibración del punto de anclaje en píxeles

    #------------------- Pantalla -----------------
    screen = pygame.display.set_mode((anchoP, altoP))
    ...
    y_anim = desplazamiento + altoP / 4 + x0_value * escala
    v_anim = v0_value
    ...
    while True:
        ...
        y_anclaje_vibracion = desplazamiento + amplitud_vibracion * np.sin(2 * np.pi * freq_vibracion * Tiempo)
        ...
        if indice_actual < len(x_esperado_list):
            y_esperado = desplazamiento + altoP / 4 + x_esperado_list[indice_actual] * escala
        ...
        posicion_texto = fuente.render(f'Posición: {(y_anim - desplazamiento - altoP / 4) / escala:.2f} m', True, texto_color)
