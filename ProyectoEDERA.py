...
def hacer_graficas():
    ...
    plt.figure("Trayectorias de Posición", figsize=(10, 5))
    ...
    plt.figure("Trayectorias de Velocidad", figsize=(10, 5))
    plt.axhline(0, color='black', linewidth=0.5)
    plt.axvline(0, color='black', linewidth=0.5)
    plt.grid(color='gray', linestyle='--', linewidth=0.5)
    for xTiempo, xValue, vValue in trayectorias:
...
def iniciar_animacion(x_tiempoPosicion, x_esperado, x_tiempo, v_esperado):
    ...
    margen_superior = 0  # píxeles desde el borde superior
    ...
    freq_vibracion = 20  # Frecuencia de vibración del punto de anclaje en Hz
    amplitud_vibracion = 3  # Amplitud de vibración del punto de anclaje en píxeles

    # Pantalla
    screen = pygame.display.set_mode((anchoP, altoP), pygame.FULLSCREEN)
    clock = pygame.time.Clock()

    # Cargar imagen del resorte
    resorte_image = pygame.image.load(r'resorte.png')
    ...
    # Colores pastel
    fondo_color_madera = (182, 88, 47)  # color madera pastel
    background_image = pygame.image.load(r'background.png')
    barra_color = (86, 86, 86)
    masa_color = (35, 35, 35)
    texto_color = (255, 255, 255)
    background_image.set_alpha(92)
    ...
    while True:
        ...
        # Dibujar el fondo
        animacion.fill((0, 0, 0))
        animacion.blit(background_image,(0, 0))
        pygame.draw.rect(animacion, fondo_color_madera, pygame.Rect(0, altoP - 100, anchoP, 100))
        pygame.draw.line(animacion, (0, 0, 0), (0, altoP - 100), (anchoP, altoP - 100), 2)
        ...
        y_anclaje_vibracion = margen_superior + amplitud_vibracion * np.sin(3 * np.pi * freq_vibracion * Tiempo)
        ...
        barra_rect = pygame.Rect(anchoP // 2 - 120, y_anclaje_vibracion, 240, 30)
        pygame.draw.rect(animacion, barra_color, barra_rect)

        # Escalar la imagen del resorte
        resorte_altura = y_anim - (y_anclaje_vibracion + 30)
        if resorte_altura > 0:
            resorte_escalado = pygame.transform.scale(resorte_image, (resorte_image.get_width(), resorte_altura))
            animacion.blit(resorte_escalado, (anchoP // 2 - resorte_image.get_width() // 2, y_anclaje_vibracion + 30))

        # Dibuja la masa
        masa_rect = pygame.Rect(anchoP // 2 - 40, y_anim, 80, 80)
        ...
        # Dibujar la sección de datos en tiempo real
